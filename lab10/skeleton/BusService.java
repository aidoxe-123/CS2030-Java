...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    CompletableFuture<Scanner> cfSc = BusAPI.getBusStopsServedBy(serviceId)
                                            .thenApply(str -> new Scanner(str));
    CompletableFuture<Set<BusStop>> cfStops = cfSc.thenApply(sc -> sc.useDelimiter("\n")
        .tokens()
        .map(line -> line.split(","))
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(stops -> stops.stream()
