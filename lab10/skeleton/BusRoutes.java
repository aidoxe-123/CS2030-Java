...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    ...
    result = result.thenApply(str -> str + "From " +  this.stop + "\n");

    for (BusService service: services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);
      result = result.thenCombine(describeService(service, stops), (s1, s2) -> s1 + s2);
    }
    return result;
  }
   ...
   *     the bus services and matching stops served.
   */
  public CompletableFuture<String> describeService(BusService service, CompletableFuture<Set<BusStop>> cfStops) {
    return cfStops.thenApply(stops -> {
      if (stops.isEmpty()) {
      return "";
      }
      return stops.stream()
